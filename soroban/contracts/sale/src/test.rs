...
struct Setup<'a> {
    ...
    pay: token::TokenClient<'a>,
    tres: token::TokenClient<'a>,
}

/// Deploys the payment-asset + TRES Stellar Asset Contracts and the sale
/// contract, funds the buyer with the payment asset and the contract with TRES.
/// `price` is payment-asset stroops per TRES.
fn setup(price: i128, buyer_pay: i128, contract_tres: i128) -> Setup<'static> {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let pay_issuer = Address::generate(&env);
    let pay_sac = env.register_stellar_asset_contract_v2(pay_issuer);
    let pay = token::TokenClient::new(&env, &pay_sac.address());
    token::StellarAssetClient::new(&env, &pay_sac.address()).mint(&buyer, &buyer_pay);
    ...
    Setup { _env: env, admin, treasury, buyer, sale_id, sale, pay, tres }
}

#[test]
fn buy_moves_payment_and_tres() {
    // 1 TRES = 50 USDC, buyer has 1000 USDC, contract holds 100 TRES.
    let s = setup(50 * ONE, 1000 * ONE, 100 * ONE);

    let cost = s.sale.buy(&s.buyer, &(2 * ONE)); // buy 2 TRES => 100 USDC

    assert_eq!(cost, 100 * ONE);
    assert_eq!(s.tres.balance(&s.buyer), 2 * ONE);
    assert_eq!(s.tres.balance(&s.sale_id), 98 * ONE);
    assert_eq!(s.pay.balance(&s.treasury), 100 * ONE);
    assert_eq!(s.pay.balance(&s.buyer), 900 * ONE);
}
