...
const UserForm = ({
  ...
  useEffect(() => {
    if (receiptSuccess && hash) {
      ...
      setError(
        "La transaction TGG a échoué. Veuillez contacter le support pour assistance."
      );
    }
  ...
  const handleApiSubmission = async () => {
    try {
      ...
      if (type === "buy") {
        ...
        const feesValue = parseFloat(
          (fiatAmount * FEES_COEF).toFixed(NUMBER_TO_FIXE_2)
        );
      ...
      const endpoint =
        type === "buy" ? "/api/transactions/buy" : "/api/transactions/sell";
      ...
      if (!response.ok) {
        console.log(data);
        throw new Error(
          data.error || "Une erreur est survenue lors de la transaction"
        );
      }
  ...
  const balanceCheck =
    type === Action.Sell && address ? checkSufficientBalance(tggAmount) : null;
      ...
      {!showConfirmation ? (
        <>
          <button
            ...
            className="bg-color4 text-white p-2 rounded-sm"
          >
            Go back
          </button>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {type === Action.Sell && (transferPending || receiptLoading) && (
              <div className="bg-yellow-50 p-4 rounded-xl">
                <p className="text-yellow-800">
                  {transferPending
                    ? "Please confirm the TGG transfer in your wallet..."
            ...
            <button
              type="submit"
              disabled={
                !isConnected || isLoading || transferPending || receiptLoading
              }
          ...
          {type === Action.Sell && (
            <div className="bg-gray-200 p-6 rounded-xl">
              <h2 className="text-xl font-bold mb-4 text-center">
                Demand of sale confirmed
              </h2>
              <p>
                Your sale operation number{" "}
                <span className="font-bold">{ref}</span> has been successfully
                processed.
              </p>
              <p className="my-3">
                ...
                on your account in a delay of 2 to 7 days, depending on the
                usual processing time of your bank.
              </p>
              <p className="my-3">
                If you encounter difficulties, please do not hesitate to contact
                us. We thank you for your trust.
              </p>
