...
export const TokenProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [swapToken, setSwapToken] = useState(() =>
    typeof window !== "undefined"
      ? localStorage.getItem("swapToken") || "USDT"
      : "USDT"
  );
  const [swapBlockchain, setSwapBlockchain] = useState<Blockchain>(() =>
    typeof window !== "undefined"
      ? (localStorage.getItem("swapBlockchain") as Blockchain) ||
        Blockchain.Polygon
      : Blockchain.Polygon
  );
  const [buyToken, setBuyToken] = useState(() =>
    typeof window !== "undefined"
      ? localStorage.getItem("buyToken") || "EUR"
      : "EUR"
  );
  const [buyBlockchain, setBuyBlockchain] = useState<Blockchain>(() =>
    typeof window !== "undefined"
      ? (localStorage.getItem("buyBlockchain") as Blockchain) ||
        Blockchain.Polygon
      : Blockchain.Polygon
  );
  const [sellToken, setSellToken] = useState(() =>
    typeof window !== "undefined"
      ? localStorage.getItem("sellToken") || "USD"
      : "USD"
  );
  const [sellBlockchain, setSellBlockchain] = useState<Blockchain>(() =>
    typeof window !== "undefined"
      ? (localStorage.getItem("sellBlockchain") as Blockchain) ||
        Blockchain.Polygon
      : Blockchain.Polygon
  );
