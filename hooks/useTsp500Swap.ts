...
export const useTsp500Swap = () => {
  ...
  const getConversion = useCallback(
    async (params: { tsp500Amount: string }) => {
  ...
  const performSwapMint = useCallback(
    async (params: {
      ...
      if (currentAllowance < amountBigInt) {
        await approveToken(params.inputToken, BASE_CONTRACTS.ZAP_TSP500 as Address, amountBigInt);
      }
  ...
  const performSwapWithdraw = useCallback(
    async (params: {
      ...
      const despxaAmountInBaseUnits = BigInt(conversionInteger).toString();
